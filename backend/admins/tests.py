...
# api_url = "https://b16c-152-58-246-191.ngrok-free.app/generate"
...
#         # Prepare the prompt data for generating the syllabus
#         data = {
#             "prompt": f"""
...
#             "api-key": "Jayasima0706",  # Replace with your API key
...
#             return Response({"error": "Failed to connect to the API."}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

#         # Decode the response content
#         response_text = response.content.decode("utf-8")
#         logger.info(f"Raw response from LLM: {response_text}")

#         # Strip the ```json markers if present
...
#             logger.error(f"JSONDecodeError: {str(e)}. Raw response: {response_text}")
#             return Response({"error": "Invalid JSON response from API."}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

#         # Return the parsed syllabus in the response
#         return Response({"syllabus": syllabus_json}, status=status.HTTP_200_OK)
# @api_view(['POST'])
