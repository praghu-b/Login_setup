...
const Enrolled = () => {
    ...
    useEffect(() => {
        if (user) {
            setIsLoading(true);
            axios.get(`http://localhost:8000/users/${user.id}/enrolled-courses/`)
                .then(response => {
    ...
    return (
        <div className="flex-1 py-4">
            <h3 className="text-2xl font-bold text-gray-800 mb-4 mt-4 px-8">Enrolled Courses</h3>
                    ...
                    <div key={course.course_id || index} className="bg-white rounded-lg shadow-lg overflow-hidden">
                        ...
                        <div className="flex justify-between p-4">
                            ...
                            <div className='my-auto'>
                                <button onClick={() => handleCoursePreview(course._id)} className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-1 rounded-2xl">Syllabus</button>
                            </div>
                        </div>
                    </div>
                ))}
            </div>
            {isPopUp && <CoursePreview syllabus={{ previewSyllabus }} setIsPopUp={setIsPopUp} status={'enrolled'} />}
        </div>
    );
}
