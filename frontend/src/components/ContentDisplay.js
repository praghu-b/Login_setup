...
import "./ContentDisplay.css"; // Import the CSS file for styling
            ...
            {Object.keys(content).map((moduleId) => (
                <div key={moduleId} className="mb-6 p-4 border rounded-lg shadow-md">
                    ...
                    {editingModule === moduleId ? (
                        ...
                        <div className="prose" dangerouslySetInnerHTML={{ __html: content[moduleId].module_content }} />
                    )}
                    <div className="flex space-x-2 mt-2">
                        <button onClick={() => handleEditModule(moduleId)} className="px-4 py-2 bg-blue-500 text-white rounded">
                            Edit
                        </button>
                        {editingModule === moduleId && (
                            <button onClick={() => handleSaveModule(moduleId)} className="px-4 py-2 bg-green-500 text-white rounded">
                                Save
                            </button>
                        )}
                        <button onClick={() => handleRegenerateModule(moduleId)} className="px-4 py-2 bg-yellow-500 text-white rounded">
                            Regenerate
                        </button>
                    </div>
                    {content[moduleId].submodules_content.map((submodule) => (
                        <div key={submodule.submodule_id} className="mt-4 p-4 border rounded-lg">
                            <h4 className="text-lg font-semibold mb-2">Submodule {submodule.submodule_id}</h4>
                            {editingModule === moduleId ? (
                                <textarea
                                    name="content"
                                ...
                                <div className="prose" dangerouslySetInnerHTML={{ __html: submodule.content }} />
                            )}
                        </div>
                    ))}
                </div>
